...
import { ClipboardList, Stethoscope, UserCheck } from 'lucide-react'; // প্রয়োজন অনুযায়ী আইকন

export function PendingPage({ go, role }: { go: (v: string) => void; role: string }) {
  return (
    <div className="min-h-screen flex items-center justify-center py-10 relative overflow-hidden"
      ...
      <div className="relative w-full max-w-md mx-4 bg-white rounded-3xl shadow-2xl p-8 text-center">
        ...
        <p className="text-gray-600 text-sm mb-8 leading-relaxed">
          {role === 'doctor' 
            ? "আপনার চেম্বারের তথ্য এবং প্রোফাইল আমাদের অ্যাডমিন টিম যাচাই করছে। ভেরিফিকেশন সফল হলে আপনি আপনার অ্যাকাউন্টে লগইন করতে পারবেন।" 
          ...
          {role === 'doctor' ? (
            <>
              <div className="flex gap-4 p-4 bg-orange-50/50 rounded-2xl border border-orange-100">
                ...
                <div>
                  <h4 className="font-bold text-gray-800 text-sm">তথ্য যাচাইকরণ</h4>
                  <p className="text-xs text-gray-500 mt-0.5">আপনার বিএমডিসি এবং চেম্বারের তথ্যগুলো চেক করা হচ্ছে।</p>
                </div>
              </div>
              <div className="flex gap-4 p-4 bg-gray-50 rounded-2xl">
                <div className="w-6 h-6 rounded-full bg-gray-300 text-white flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">২</div>
                <div>
                  <h4 className="font-bold text-gray-800 text-sm">অ্যাডমিন অনুমোদন</h4>
                  <p className="text-xs text-gray-500 mt-0.5">অনুমোদন সম্পন্ন হওয়া মাত্রই আপনার প্রোফাইল অ্যাক্টিভ হবে।</p>
                </div>
              </div>
            </>
          ) : (
            // এজেন্টদের জন্য আগের ডিজাইন
            <>
              <div className="flex gap-4 p-4 bg-orange-50/50 rounded-2xl border border-orange-100">
                <div className="w-6 h-6 rounded-full bg-orange-500 text-white flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">১</div>
                <div>
                  <h4 className="font-bold text-gray-800 text-sm">ডকুমেন্ট ভেরিফিকেশন</h4>
                  <p className="text-xs text-gray-500 mt-0.5">আমরা আপনার সিভি ও সার্টিফিকেট যাচাই করব।</p>
                </div>
              </div>
              <div className="flex gap-4 p-4 bg-gray-50 rounded-2xl">
                <div className="w-6 h-6 rounded-full bg-gray-300 text-white flex items-center justify-center text-xs font-bold flex-shrink-0 mt-0.5">২</div>
                <div>
                  <h4 className="font-bold text-gray-800 text-sm">ইন্টারভিউ</h4>
                  <p className="text-xs text-gray-500 mt-0.5">ভেরিফিকেশন শেষে আপনাকে ইন্টারভিউয়ের জন্য ডাকা হবে।</p>
                </div>
              </div>
            </>
          )}
